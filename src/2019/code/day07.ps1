...
function Invoke-Program ([long[]]$ram, [long]$phase, [long]$power)
    {   
        $pc = 0
        $PhaseApplies = $false
        while ($true) {
...
foreach ($perm in $perms) {
    $output = 0
    foreach ($i in (0..4)) {
        $output = Invoke-Program -ram $cleanRam.Clone() -phase $perm[$i] -power $output
    }
    $powers += $output
}

$maxPower1 = ($powers | measure -Maximum).Maximum

$script:perms = @()
Get-Permutations -things @(5..9)

Write-Host "Part 1: $maxPower1"
Write-Host "Part 2: "
$timer.Stop()
Write-Host "Runtime:"
