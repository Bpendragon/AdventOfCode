...
namespace Day11
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bot.RunProgram(true);
            SortedDictionary<int, SortedSet<int>> WhiteTiles = new SortedDictionary<int, SortedSet<int>>();
            ...
            foreach(var t in bot.PaintedTiles)
            {
                if (t.Value == 1)
                {
                    var s = t.Key.Split(',');
                    ...
                    if (y < minY) minY = y;
                    if (y > maxY) maxY = y;
                    if(!WhiteTiles.ContainsKey(x))
                    {
                        WhiteTiles[x] = new SortedSet<int>();
                    }
                    WhiteTiles[x].Add(y);
                }
            }
            ...
            {
                string line = "";
                if (!WhiteTiles.ContainsKey(i)) continue;
                for (int j = minY; j <= maxY; j++)
                { 
                    if (WhiteTiles[i].Contains(j)) line += "*";
                    else line += " ";
                }
    ...
    public class PaintBot
    {
        ...
        public void RunProgram(bool part2 = false)
        {
            ...
            while (isProcessing)
            {
                ...
                while (outPutStream.Count == 0 && isProcessing) { }
